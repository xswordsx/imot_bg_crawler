...
class ImotBgCrawlerPipeline:
    ...
    def get_filename(prefix="result_", suffix=None):
        return (
            f'{prefix}{datetime.datetime.now().strftime("%d-%m-%Y@%Hh%Mm")}'
    ...
    def get_common_images_folder(self):
        common_folder = os.path.join(
            self.output_path, self.items_folder, "all_pictures"
        )
    ...
    def close_spider(self, spider):
        ...
        self.results_file.write(
            json.dumps(self.result, ensure_ascii=False).encode("utf8").decode()
        )
